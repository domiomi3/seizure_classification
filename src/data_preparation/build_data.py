...
parameters = pd.read_csv('/src/data_preparation/parameters.csv', index_col=['parameter'])
...
def generate_data_dict(xlsx_file_name,sheet_name, tuh_eeg_szr_ver):
    ...
    if tuh_eeg_szr_ver == 'v1.5.2':
        ...
        data[col_o_szr_type] = data[col_o_szr_type].fillna("NOSEIZ")
        # replace nan with "NOSEIZ" type for further classification

        data = data.iloc[1:]
        train_files = data[[col_l_file_name, col_m_start,col_n_stop,col_o_szr_type]]

        for item in train_files.values:
            elements = item[0].split('/')
            if tuh_eeg_szr_ver == 'v1.5.2':
                patient_id = elements[4]
            else:
                exit('tuh_eeg_szr_ver %s is not supported' % tuh_eeg_szr_ver)

            if item[3] == "NOSEIZ":
                path = os.path.join("/home/dominika/ML Projects/MSc/seizure_detection/data/raw/v1.5.2/edf/", item[0][2:])
...
def print_type_information(data_dict):
    ...
    for szr_type, szr_info_list in data_dict.items():
        ...
        dur_list = [float(szr_info.end_time) - float(szr_info.start_time) for szr_info in szr_info_list]
...
def main():
    ...
    if platform.system() == 'Linux':
        parser.add_argument('-b', '--base_dir', default='/slow1/raw_datasets/tuh/tuh_eeg_seizure/',
                            help='path to raw seizure dataset')
        parser.add_argument('-s', '--save_data_dir', default='/slow1/out_datasets/tuh/seizure_type_classification/',
